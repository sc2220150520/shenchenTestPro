...
#import "UIImage+LDResize.h"
#import "UIImage+LDAlpha.h"

static CGRect ld_swapWidthAndHeight(CGRect rect)
{
...
@implementation UIImage (LDResize)
...
{
    UIImage *resizedImage = [self ld_resizedImageWithContentMode:UIViewContentModeScaleAspectFill
    ...
    CGRect cropRect = CGRectMake(round((resizedImage.size.width - thumbnailSize) / 2),
                                 ...
                                 thumbnailSize,
                                 thumbnailSize);
    UIImage *croppedImage = [resizedImage ld_croppedImage:cropRect];
    
    UIImage *transparentBorderImage = borderSize ? [croppedImage ld_transparentBorderImage:borderSize] : croppedImage;
    
    return [transparentBorderImage ld_roundedCornerImage:cornerRadius borderSize:borderSize];
}
...
- (UIImage *)ld_resizedImage:(CGSize)newSize interpolationQuality:(CGInterpolationQuality)quality
{
    ...
    return [self ld_resizedImage:newSize
                       transform:[self ld_transformForOrientation:newSize]
...
- (UIImage *)ld_resizedImageWithContentMode:(UIViewContentMode)contentMode bounds:(CGSize)bounds interpolationQuality:(CGInterpolationQuality)quality
{
    ...
    return [self ld_resizedImage:newSize interpolationQuality:quality];
}
...
- (UIImage *)ld_resizedImage:(CGSize)newSize transform:(CGAffineTransform)transform drawTransposed:(BOOL)transpose interpolationQuality:(CGInterpolationQuality)quality
{
...
- (CGAffineTransform)ld_transformForOrientation:(CGSize)newSize
{
...
- (UIImage *)ld_rotate:(UIImageOrientation)orient
{
    ...
    switch (orient)
    {
        ...
        case UIImageOrientationLeft:
            bnds = ld_swapWidthAndHeight(bnds);
...
{
    CGAffineTransform transform = [self ld_transformForOrientation:newSize];
...
- (UIImage *)ld_roundedCornerImage:(NSInteger)cornerSize borderSize:(NSInteger)borderSize
{
    ...
    UIImage *image = [self ld_imageWithAlpha];
    ...
    [self ld_addRoundedRectToPath:CGRectMake(borderSize, borderSize, image.size.width - borderSize * 2, image.size.height - borderSize * 2)
                          context:context
...
// Original author: Björn Sållarp. Used with permission. See: http://blog.sallarp.com/iphone-uiimage-round-corners/
- (void)ld_addRoundedRectToPath:(CGRect)rect context:(CGContextRef)context ovalWidth:(CGFloat)ovalWidth ovalHeight:(CGFloat)ovalHeight
{
...
- (UIImage *)ld_stretchAndRetinaImage
{
    if ([self respondsToSelector:@selector(scale)]) {
        UIImage *newImage = [UIImage imageWithCGImage:self.CGImage scale:2.0f orientation:self.imageOrientation];
        return [newImage stretchableImageWithLeftCapWidth:self.size.width / 4 topCapHeight:self.size.height / 4];
    }
    return [[self ld_resizeImageWithNewSize:CGSizeMake(self.size.width / 2, self.size.height / 2)]
            stretchableImageWithLeftCapWidth:self.size.width / 4 topCapHeight:self.size.height / 4];
