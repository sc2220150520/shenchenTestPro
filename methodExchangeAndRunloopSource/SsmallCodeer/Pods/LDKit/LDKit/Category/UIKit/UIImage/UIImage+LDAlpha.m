...
@implementation UIImage (LDAlpha)
...
- (UIImage *)ld_transparentBorderImage:(NSUInteger)borderSize
{
    ...
    CGImageRef maskImageRef = [self ld_newBorderMask:borderSize size:newRect.size];
...
- (CGImageRef)ld_newBorderMask:(NSUInteger)borderSize size:(CGSize)size
{
...
{
    return [self ld_maskWithColor:color shadowColor:[UIColor blackColor] shadowOffset:CGSizeZero shadowBlur:0];
}
...
- (UIImage *)ld_imageTintedWithColor:(UIColor *)color
{
    ...
    return [self ld_imageTintedWithColor:color fraction:0.0]; // default to a fully tinted mask of the image.
}
...
{
   return [self ld_imageWithBackgroundColor:color toSize:CGSizeMake(1, 1) cornerRadius:0 borderColor:nil borderWidth:0];
}
