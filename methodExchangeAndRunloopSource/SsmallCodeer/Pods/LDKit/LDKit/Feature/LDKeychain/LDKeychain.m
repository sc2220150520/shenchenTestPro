...
#import "LDKeychain.h"
#import "LDKeyChainConstants.h"
#import <Security/Security.h>
#import <CommonCrypto/CommonDigest.h>

@implementation LDKeychain
...
// *** END NOTE ***
+ (NSMutableDictionary *)ld_setupSearchDirectoryForIdentifier:(NSString *)identifier {
...
+ (NSData *)ld_searchKeychainCopyMatchingIdentifier:(NSString *)identifier {
    ...
    NSMutableDictionary *searchDictionary = [self ld_setupSearchDirectoryForIdentifier:identifier];
...
+ (NSString *)ld_keychainStringFromMatchingIdentifier:(NSString *)identifier {
    NSData *valueData = [self ld_searchKeychainCopyMatchingIdentifier:identifier];
    if (valueData) {
...
+ (BOOL)ld_createKeychainValue:(NSString *)value forIdentifier:(NSString *)identifier {
    ...
    NSMutableDictionary *dictionary = [self ld_setupSearchDirectoryForIdentifier:identifier];
    ...
    if (status == errSecSuccess) {
        ...
        return [self ld_updateKeychainValue:value forIdentifier:identifier];
    } else {
        return NO;
    }
}
...
+ (BOOL)ld_compareKeychainValueForMatchingPIN:(NSUInteger)pinHash {
    ...
    if ([[self ld_keychainStringFromMatchingIdentifier:PIN_SAVED] isEqualToString:[self ld_securedSHA256DigestHashForPIN:pinHash]]) {
        return YES;
    } else {
        return NO;
    }
...
+ (NSString *)ld_securedSHA256DigestHashForPIN:(NSUInteger)pinHash {
    ...
    // 3
    NSString *finalHash = [self ld_computeSHA256DigestForString:computedHashString];
