...
@implementation NSData (LDCrypto)
...
- (NSData *)ld_AES256EncryptWithKey:(NSString *)key
{
    ...
    NSData *keyData = [key ld_base16Data];
...
- (NSData *)ld_getSignatureBytes:(SecKeyRef)prikey
{
    ...
    SecKeyRawSign(	prikey,
                  kSecPaddingPKCS1,
                  (const uint8_t *)[[self ld_MD5] bytes],
...
- (BOOL)ld_verifySignature:(SecKeyRef)publicKey signature:(NSData *)sig
{
    size_t signedHashBytesSize = 0;
